...
class TestAccountsRouter:
    ...
    async def test_register_user_success(self, test_client, mock_mongodb):
        ...
        user_id = str(uuid4())
        mock_mongodb.users.insert_one.return_value = AsyncMock(inserted_id=ObjectId())
        created_user = TestFixtures.create_test_user(user_id=user_id)
        mock_mongodb.users.find_one.return_value = created_user
    ...
    async def test_register_user_email_exists(self, test_client, mock_mongodb):
        ...
        mock_mongodb.users.find_one.return_value = existing_user
    ...
    async def test_login_success(self, test_client, mock_mongodb):
        ...
        mock_mongodb.users.find_one.return_value = user_data
        ...
        with patch('core.security.verify_password') as mock_verify, \
             patch('core.jwt.create_access_token') as mock_create_token:
    ...
    async def test_logout_success(self, authenticated_client):
        ...
        assert response.status_code == 200
        assert "logged out successfully" in response.json()["message"].lower()    @pytest.mark.asyncio
    ...
    async def test_request_password_reset_user_not_found(self, test_client, mock_mongodb):
        ...
        assert response.status_code == 200
        assert "password reset instructions sent" in response.json()["message"].lower()
    ...
    async def test_confirm_password_reset_invalid_token(self, test_client, mock_mongodb):
        ...
        assert response.status_code == 400
        assert "invalid or expired" in response.json()["detail"].lower()    @pytest.mark.asyncio
