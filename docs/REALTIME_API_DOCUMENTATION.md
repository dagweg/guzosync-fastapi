...
GuzoSync provides comprehensive real-time functionality through WebSocket connections, enabling live updates for bus tracking, chat messaging, and notifications. This document provides complete integration guidelines for frontend developers.
...
- Invalid tokens result in connection closure with code `4001`
- Token must be valid and not expired

### Error Codes
- `4001`: Authentication failed
- `4002`: Connection error
...
```javascript
{
    "type": "room_joined",
    "room_id": "conversation:123",
    "message": "Joined room conversation:123"
}
```

#### 2. Room Left Confirmation
```javascript
{
    "type": "room_left",
    "room_id": "conversation:123",
    "message": "Left room conversation:123"
}
```
...
class GuzoSyncWebSocket {
    ...
    connect() {
        try {
            this.ws = new WebSocket(`ws://localhost:8000/ws/connect?token=${this.token}`);
